...
func main() {
	...
	if err = (&cdpipeline.ReconcileCDPipeline{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
		Log:    ctrlLog.WithName("cd-pipeline"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "cd-pipeline")
		os.Exit(1)
	}

	rs := &stage.ReconcileStage{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
		Log:    ctrlLog.WithName("cd-stage"),
	}
	if err := rs.AddIndex(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "cd-stage")
		os.Exit(1)
	}
	if err := rs.SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "cd-stage")
		os.Exit(1)
	}
